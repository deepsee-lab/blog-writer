<template>
  <div v-loading="ParentLoading" element-loading-text="生成中..." class="">
    <!-- 游记大纲标题 -->
    <h1 class="text-4xl font-bold text-gray-800 mb-6 border-b-2 border-gray-200 pb-2">
      {{ content.title }}
    </h1>
    ...
    <div class="overflow-y-auto max-h-[70vh] pr-4">
      <!-- 遍历段落 -->
      <div v-for="(item, index) in content.content.body" :key="index"
        class="mb-10 pb-6 border-b border-gray-200 last:border-none">
        <!-- 小节标题 -->
        <h2
        ...
        <!-- 小节内容 -->
        <p style="text-indent: 2em;" class="max-w-[900px] mx-auto text-base text-left text-gray-600 leading-relaxed">
          {{ item.content }}
        </p>
      </div>
    </div>
