...
from src.api.backend.self_media.apps.travel.scripts import travel_generate as travel_generate
...
def get_frame_work(request_data: dict = {
    ...
    try:
        result = travel_generate.generate_travel_frame_work(
            theme=theme, travel_descriptions=travel_descriptions, image_descriptions=image_descriptions, type_name="openai", model_select="gpt-4o", url=url, temperature=0.3
        )
        print("travel_generate result:", result)
...
def md2json_test(request_data: dict = {
    ...
    try:
        ...
        if not markdown_file:
            return Response(
                code="400",
                message="Missing required parameter: 'markdown_file'.",
                data={}
            )
    ...
    # 调用转换逻辑
    try:
        # 调用 `travel_generate.md2json` 方法，传入必要参数
        result = travel_generate.md2json(
            markdown_file,
        ...
        if not result or not isinstance(result, dict):
            return Response(
                code="500",
                message="Invalid response from md2json processing.",
                data={}
            )

        # 将结果转换为 JSON 格式
        json_output = result.get("data", {})
        if not json_output:
            return Response(
                code="500",
                message="No data found in md2json response.",
                data={}
            )
        ...
        with open(output_file, "w", encoding="utf-8") as file:
            file.write(json.dumps(json_output, ensure_ascii=False, indent=4))  # 格式化 JSON 输出
        ...
        # 记录错误日志并返回错误响应
        logger.error(f"Error in processing md2json: {str(e)}")
        return Response(
            code="500",
            message=f"Internal server error: {str(e)}",
            data={}
